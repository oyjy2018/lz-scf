...
import com.zhjs.scfcloud.app.feign.CreditTicketApplyFeignService;
    ...
    public Result findCreditTicketCfg(QueryCreditTicketCfgDTO dto) {
        ...
        if(dto.getCreditTicketApplyId() == null){
            ...
            if(dto.getCreditRecordId() == null){
                Result<List<PersonalCreditVO>> result = creditRecordFeignService.findPersonalCreditList(user.getIdCard(),MySubjectUtil.getCompanyId());
        ...
        //排除登录用户没有权限的流程扭转配置
        List<BusinessWorkFlowExtendVO> trailingWorkFlowCfg = BusinessCfgUtil.powerFilterWorkFlowCfg((List<BusinessWorkFlowExtendVO>)data.get("trailingWorkFlowCfg"), user.getId(), MySubjectUtil.getRoleList());
