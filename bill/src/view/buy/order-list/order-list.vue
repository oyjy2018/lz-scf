    ...
    <Card :bordered="false">
      <div class="search-con-top">
        <Input clearable
               placeholder="输入卖方（票方）"
               class="search-input"
               v-model="form.publishCompanyName"
               style="width: 200px" />
        <Input clearable
               placeholder="输入订单ID"
               class="search-input"
          ...
          <div @mouseenter="show(index)">
            <Dropdown placement="right">
              ...
              <DropdownMenu slot="list">
                <DropdownItem v-if="row.orderStatus === 1 && !row.hasUserSign">
                  <Button @click="handleSignContract(row, 1)"
                          type="text">授权人签署合同</Button>
                </DropdownItem>
                <DropdownItem v-if="row.orderStatus === 1 && !row.hasCompanySign">
                  <Button @click="handleSignContract(row, 0)"
                          type="text">企业签署合同</Button>
                </DropdownItem>
                <DropdownItem v-if="(row.orderStatus !== 1 || (row.hasUserSign || row.hasCompanySign)) && row.orderStatus !== 97 && row.orderStatus !== 98">
                  <Button @click="handleViewContractFile(row)"
                          type="text">查看合同</Button>
                </DropdownItem>
                <DropdownItem v-if="row.orderStatus === 3 && row.payStatus !== 97">
                  <Button @click="handlePay(row)"
                          type="text">{{row.payStatus === 1 ? '继续支付' : '支付'}}</Button>
                </DropdownItem>
...
export default {
  ...
  data () {
    return {
      ...
      form: {
        ...
        dealEndDate: '',
        publishCompanyName: '',
        acceptorName: this.$route.query.acceptorName,
        isTicketMaturity: this.$route.query.isTicketMaturity // 商票是否过期
      },
      activeIndex: -1,
      total: 0,
      list: [],
  ...
  methods: {
    ...
    handleSignContract (row, type) {
      if (row.orderStatus === 1 && !row.hasCompanySign && !row.hasUserSign) {
        const route = {
        ...
        this.showModal = true
        this.$refs.agreement.getContent(row.id, type, row.contractType, row.contractFileUrlView)
      }
    },
    handleClose (src) {
    ...
    handlePay (row) {
      ...
      this.payOrder(data).then(res => {
        if (res && res.status === 200 && res.data && res.data.code === 10000) {
          window.open(res.data.data.cashierDeskUrl, '_blank')
        }
      })
    },
    handleRevoke (row) {
