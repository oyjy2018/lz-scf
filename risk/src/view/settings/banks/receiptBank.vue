  ...
  <div class="list-table">
            ...
            <Dropdown placement="bottom-start">
              ...
              <DropdownMenu slot="list">
                ...
                <DropdownItem v-if="row.accountStatus === 2">
                  <Button :disabled="row.isReceiptDefault ? true : false"
                          @click="handleSetDefault(row.id, '1')"
                          type="text">设为默认收票账户</Button>
                </DropdownItem>
    ...
    <Modal v-model="modal"
      ...
      <div>
        ...
        <br>
        <p>提示：验证成功后，您还需去企业网银授权ECDS查询，收票账户才能添加成功</p>
        <p><a href="../../file/企业平安银行账户ECDS授权操作指引.docx"
             target="_blank">企业平安银行账户ECDS授权操作指引.docx</a></p>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      this.getCompanyBanksReceipt(this.form).then(res => {
    ...
    handleVerifySubmit () {
      if (this.amount !== '') {
        ...
        this.verifyCompanyBankReceipt(data).then(res => {
          if (res && res.status === 200 && res.data.code === 10000) {
    ...
    handleSetDefault (companyBankId, accountType) {
      this.defaultCompanyBankReceipt({ companyBankId, accountType }).then(res => {
        if (res && res.status === 200 && res.data.code === 10000) {
    ...
    handleSendBankPayment (companyBankId) {
      this.sendBankPaymentReceipt({ companyBankId }).then(res => {
        if (res && res.status === 200 && res.data.code === 10000) {
    ...
    handleDelete (companyBankId) {
      ...
      this.$Modal.confirm({
        ...
        content: '<p>是否确认删除？</p>',
        onOk: () => {
          _this.deleteCompanyAccountTypeReceipt({ companyBankId, accountType: 1 }).then(res => {
            if (res && res.status === 200 && res.data.code === 10000) {
    ...
    routerTo () {
      this.$router.push({ path: './add', query: { accountType: 1 } })
    },
    changePage (current) {
