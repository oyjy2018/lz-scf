        ...
        <Affixs :offset-top="104"
          ...
          <div class="tabs-items">
            ...
            <div class="tabs-item"
                 ...
                 @click="onClickTab('1')">申请详情、审核详情</div>
            <div class="tabs-item"
                 :class="{'tabs-item-active': accordion === '2'}"
                 @click="onClickTab('2')">审核日志</div>
            <div class="tabs-item"
                  ...
                  <div class="form-steps"
                    ...
                    <div class="form-steps-item sention-sub"
                      ...
                      <Affixs :offset-top="index * 30 + 151"
                              v-show="scrollTop < (sention1 + (index * 50) - 600)"
                              container=".content-wrapper">
                      ...
                      <div class="form-steps-context">
                        <div v-for="(subItem, subIndex) in item.attrList"
                          ...
                          <div class="form-value"
                               v-if="subItem.attrValObj">{{subItem.attrValObj['attrValMap'][subItem.columnVal]['valueCh']}} {{subItem.columnUnit}}</div>
                          <div class="form-value"
                               v-else>{{subItem.columnVal}} {{subItem.columnUnit}}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="form-steps"
                    ...
                    <div class="form-steps-item approval-sub"
                      ...
                      <Affixs :offset-top="index * 44 + 151"
                              ...
                              container=".content-wrapper">
                        <div class="form-steps-title"
                             @click="collapse(1,index)">
                          <div class="form-steps-title-num">
                            <div><span :class="{'span-active': index === subStep[1]}">{{index+1}}</span></div>
                          </div>
                          <div class="form-steps-title-text">
                            <div style="line-height: 30px"><span>{{item.auditBeforeFlowCH}}</span></div>
                      ...
                      <div class="form-steps-context"
                        ...
                        <div v-for="(subItem, subIndex) in item.applyColumnList"
                             ...
                             style="padding-left: 180px;display:flex;line-height: 34px;">
                          <div class="form-label">{{subItem.columnCh}}</div>
                          <font v-if="subItem.required"
                                color="red">*</font>
                          <div class="form-value">
...
import { deepCopy, scrollTop } from '@/libs/util.js'
...
export default {
  ...
  mounted () {
    on(document.querySelector('.content-wrapper'), 'scroll', this.handleScroll)
    on(document.querySelector('.content-wrapper'), 'resize', this.handleScroll)
  },
  beforeDestroy () {
    off(document.querySelector('.content-wrapper'), 'scroll', this.handleScroll)
    off(document.querySelector('.content-wrapper'), 'resize', this.handleScroll)
  },
  methods: {
    ...mapActions([
    ...
    scrollTopEl (index) {
      ...
      scrollTop(document.querySelector('.content-wrapper'), this.scrollTop, to, 1000, () => {
    ...
    handleScroll () {
      this.sention = document.querySelectorAll('.sention')
      this.scrollTop = document.querySelector('.content-wrapper').scrollTop
      document.querySelectorAll('.sention').forEach((item, index) => {
        if (this.scrollTop >= item.offsetTop - this.sention[0].offsetTop) {
          this.activeStep = index
        }
      })
      document.querySelectorAll('.sention-sub').forEach((item, index) => {
        if (this.scrollTop >= item.offsetTop - this.sention[0].offsetTop) {
          this.subStep[0] = index
        }
      })
      document.querySelectorAll('.approval-sub').forEach((item, index) => {
        if (this.scrollTop >= item.offsetTop - this.sention[0].offsetTop) {
          this.subStep[1] = index
        }
      })
      // 判断是否滚到底部 element.scrollHeight - element.scrollTop === element.clientHeight
      if ((this.scrollTop + document.querySelector('.content-wrapper').clientHeight) === document.querySelector('.content-wrapper').scrollHeight) {
