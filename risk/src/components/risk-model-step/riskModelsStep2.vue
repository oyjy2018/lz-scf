<template>
  <div>
    ...
    <Button icon="md-add"
            ...
            :to="{path:'./add-score',query:{id:this.modelId,type:'addModel'}}">添加评分项
    </Button>
...
export default {
  ...
  methods: {
    ...mapActions(['modelItem', 'removeScore', 'updatafullMark']),
    ...mapMutations([
      'closeTag'
    ]),
    handleInput (event) {
    ...
    getModelItemnData: function () {
      this.modelItem(this.modelId).then(res => {
    ...
    next () {
      if (this.data1.length && this.data1.length > 0) {
        this.cancel()
        setTimeout(() => {
          this.$router.push({ path: '/riskModel/riskControl/model/add-model', query: { id: this.modelId, current: 2 } })
        }, 10)
      } else {
        this.$Message.error('至少添加一项评分项！')
      }
    },
    cancel () {
      this.closeTag({
